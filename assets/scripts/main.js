...
function initializeServiceWorker() {
  ...
  if ('serviceWorker' in navigator) {
    ...
    window.addEventListener('load', function () {
      ...
      var register = navigator.serviceWorker.register('./sw.js');
      // B4. TODO - Once the service worker has been successfully registered, console
      //            log that it was successful.

      register.then(res => console.log( `success`))
  ...
  return new Promise(async (resolve, reject) => {
    ...
    for (var url of RECIPE_URLS) {
      ...
      try {
        ...
        //            "await" again
        var new_recipe = await p.json();
        // A8. TODO - Add the new recipe to the recipes array

        arr_recipes.push(new_recipe);
        ...
        //            resolve() method.
        window.localStorage.setItem('recipes', JSON.stringify(arr_recipes));
